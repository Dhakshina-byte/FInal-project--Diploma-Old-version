...
namespace FInal_project
{
    public partial class Add_Service_Head : Form
    {
        ...
        public void Data()
        {
            SqlCommand cmd = new SqlCommand("SELECT Employee.emp_id , Employee.emp_fname ,Employee.emp_lname,Employee.NIC_ID,Employee.E_mail,Employee.DOB, Employee.mobile_number, Employee.gender,Employee.EAddress,jobTitle.jobname,Users. username,Users.passwords,Department.department_name,Department.location_name FROM Employee INNER JOIN jobTitle  ON  Employee.jobId = jobTitle.jobId INNER JOIN Users ON Employee.userID = Users.userID INNER JOIN Department ON Employee.department_ID = Department.department_id  WHERE jobTitle.jobname ='Service Head' ;", con);
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            {
                SqlCommand cmd = new SqlCommand("SELECT Employee.emp_id , Employee.emp_fname ,Employee.emp_lname,Employee.NIC_ID,Employee.E_mail,Employee.DOB, Employee.mobile_number, Employee.gender,Employee.EAddress,jobTitle.jobname,Users. username,Users.passwords,Department.department_name,Department.location_name FROM Employee INNER JOIN jobTitle  ON  Employee.jobId = jobTitle.jobId INNER JOIN Users ON Employee.userID = Users.userID INNER JOIN Department ON Employee.department_ID = Department.department_id  WHERE jobTitle.jobname ='Service Head'AND Employee.emp_id = @EID", con);
                SqlDataAdapter da = new SqlDataAdapter(cmd);
                EID = Convert.ToInt32(textBox8.Text);
                cmd.Parameters.AddWithValue("EID", EID);
                con.Open();
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            add();
            cmd.ExecuteNonQuery();
            cmd = new SqlCommand("INSERT INTO Employee(emp_id, emp_fname,emp_lname,NIC_ID, E_mail,DOB,mobile_number, gender,userID,department_ID,jobID,EAddress)VALUES (@EID,@Fname,@Lname,@NIC,@Email,@DOB,@Moblie,@gender,@usi,3,5,@address);", con);
            add();
            cmd.ExecuteNonQuery();
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            int userID = Convert.ToInt32(dt.Rows[0][0]);
            newUserID = userID;
            cmd = new SqlCommand("Update Users set username=@UN,passwords=@PW,jobId =5 where userID = @usi", con);
            add();
            cmd.ExecuteNonQuery();
            cmd = new SqlCommand("Update Employee set emp_fname=@Fname,emp_lname=@Lname,NIC_ID=@NIC,E_mail=@Email,DOB=@DOB,mobile_number=@Moblie,gender=@gender,userID=@usi,jobId =5,department_ID=3,EAddress=@address where emp_id = @EID;", con);
            add();
            cmd.ExecuteNonQuery();
            Data();
            clear();
