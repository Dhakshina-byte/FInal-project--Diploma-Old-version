...
{
    public partial class Add_Inventory_Manager : Form
    {
        public Add_Inventory_Manager()
        {
            InitializeComponent();
        }
        ...
        public void Data()
        {
            SqlCommand cmd = new SqlCommand("SELECT Employee.emp_id , Employee.emp_fname ,Employee.emp_lname,Employee.NIC_ID,Employee.E_mail,Employee.DOB, Employee.mobile_number, Employee.gender,Employee.EAddress,jobTitle.jobname,Users. username,Users.passwords,Department.department_name,Department.location_name FROM Employee INNER JOIN jobTitle  ON  Employee.jobId = jobTitle.jobId INNER JOIN Users ON Employee.userID = Users.userID INNER JOIN Department ON Employee.department_ID = Department.department_id  WHERE jobTitle.jobname ='Inventory Manager' ;", con);
        ...
        private void Add_Inventory_Manager_Load(object sender, EventArgs e)
        {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            add();
            cmd.ExecuteNonQuery();
            cmd = new SqlCommand("INSERT INTO Employee(emp_id, emp_fname,emp_lname,NIC_ID, E_mail,DOB,mobile_number, gender,userID,department_ID,jobID,EAddress)VALUES (@EID,@Fname,@Lname,@NIC,@Email,@DOB,@Moblie,@gender,@usi,1,3,@address);", con);
            add();
            cmd.ExecuteNonQuery();
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            int userID = Convert.ToInt32(dt.Rows[0][0]);
            newUserID = userID;
            cmd = new SqlCommand("Update Users set username=@UN,passwords=@PW,jobId =3 where userID = @usi", con);
            add();
            cmd.ExecuteNonQuery();
            cmd = new SqlCommand("Update Employee set emp_fname=@Fname,emp_lname=@Lname,NIC_ID=@NIC,E_mail=@Email,DOB=@DOB,mobile_number=@Moblie,gender=@gender,userID=@usi,jobId =3,department_ID=1,EAddress=@address where emp_id = @EID;", con);
            add();
            cmd.ExecuteNonQuery();
            Data();
            clear();
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            {
                SqlCommand cmd = new SqlCommand("SELECT Employee.emp_id , Employee.emp_fname ,Employee.emp_lname,Employee.NIC_ID,Employee.E_mail,Employee.DOB, Employee.mobile_number, Employee.gender,Employee.EAddress,jobTitle.jobname,Users. username,Users.passwords,Department.department_name,Department.location_name FROM Employee INNER JOIN jobTitle  ON  Employee.jobId = jobTitle.jobId INNER JOIN Users ON Employee.userID = Users.userID INNER JOIN Department ON Employee.department_ID = Department.department_id  WHERE jobTitle.jobname ='Inventory Manager'AND Employee.emp_id = @EID", con);
                SqlDataAdapter da = new SqlDataAdapter(cmd);
